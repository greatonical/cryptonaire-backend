...
export class GameService {
  ...
  private static readonly GEN_PROMPT: Record<Stage, string> = {
    [STAGES.BASIC]: "Beginner crypto literacy, fundamentals, wallet basics.",
    [STAGES.MID]:
  ...
  private async bumpLeaderboards(userId: string, delta: number) {
    ...
    const clamp = async (key: string) => {
      const newScore = Number(await r.zincrby(key, delta, userId));
      if (newScore < 0) {
        // clamp to 0 so the board never shows negative points
        await r.zadd(key, 0, userId);
        return 0;
      }
      return newScore;
    };

    await Promise.all([clamp(weeklyKey(wk)), clamp(ALL_TIME_KEY)]);
  }
  ...
  async nextQuestion(userId: string) {
    ...
    if (!question) {
      ...
      return this.buildAttemptAndResponse(session.id, userId, retry, stage);
    }

    return this.buildAttemptAndResponse(session.id, userId, question, stage);
  }
  ...
  async submitAttempt(userId: string, dto: SubmitAttemptDto) {
    ...
    try {
      ...
      const stageComplete =
        (stageState.byStage[s]?.answered ?? 0) >= STAGE_QUESTIONS_REQUIRED[s] &&
      ...
      const gameComplete = [STAGES.BASIC, STAGES.MID, STAGES.ADV].every(
        (st) => {
          ...
          return (
            a >= STAGE_QUESTIONS_REQUIRED[st] && c >= STAGE_PASS_THRESHOLD[st]
          );
        }
      );
  ...
  async continueWithPenalty(userId: string, reason?: string) {
    ...
    // decide stage as you already do
    const decided =
      (await this.decideCurrentStage(
        session.id,
        this.asStage(session.stageUnlocked)
    ...
    // compute current total positive points in the session
    const state = await this.computeStageState(session.id);
    const currentTotal =
      Math.max(0, state.byStage[STAGES.BASIC]?.points ?? 0) +
      Math.max(0, state.byStage[STAGES.MID]?.points ?? 0) +
      Math.max(0, state.byStage[STAGES.ADV]?.points ?? 0);

    // base penalty from config, but don't exceed what the user has
    const base = Math.abs(STAGE_PENALTY[decided] ?? 1);
    const penalty = -Math.min(currentTotal, base); // never push below 0

    // find any question in the decided category (for FK integrity)
    const anyQ = await this.prisma.question.findFirst({
      where: { category: { equals: cat, mode: "insensitive" }, active: true },
      select: { id: true },
    });
    if (!anyQ)
      throw new BadRequestException(
    ...
    // leaderboard also clamped by bumpLeaderboards()
    await this.bumpLeaderboards(userId, penalty);
  ...
  private async computeStageState(sessionId: string) {
    ...
    for (const s of [STAGES.BASIC, STAGES.MID, STAGES.ADV] as const) {
      if (byStage[s].points < 0) byStage[s].points = 0;
    }
  ...
  private async ensurePoolForStage(stage: Stage) {
    ...
    this.logger.debug(`ensurePoolForStage(${stage}/${cat}): active=${current}`);
